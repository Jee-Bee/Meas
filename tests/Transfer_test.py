...
#noise = rand(1,length(delsamp) + length(out))*1/(2^16);
...
F=np.arange(fs/(len(out)+1),fs/2,fs/(len(out)+1)) #F = [0 F]  # add zero for first signal
...
val =(len(IN1)/2);
#IN1test = IN1[val::]
plt.subplot(2,1,1); plt.semilogx(F,abs(IN1[val:]));# hold all;
plt.subplot(2,1,1); plt.semilogx(F,abs(IN2[val:]))
plt.subplot(2,1,1); plt.semilogx(F,abs(IN3[val:]))
plt.subplot(2,1,1); plt.semilogx(F,abs(OUT[val:]))
